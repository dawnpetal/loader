...
local function createSection(parent, title, layoutOrder, defaultExpanded, isWide, infoText)
	...
	local function validateGridPosition(columns, rows)
		if not columns or columns < 1 or columns > 2 then
			return false
		end
		if not rows or rows < 1 or rows > 3 then
			return false
		end
		return true
	end
	...
	local function calculateContentHeight()
		if isWide then
			local childCount = # gridComponents
			if childCount <= 0 then
				return 10
			end
			return (childCount * 50) + 10
		else
			local maxRow = 0
			local maxRowHeight = {}
			for _, componentData in pairs(gridComponents) do
				local endRow = componentData.row + componentData.rows - 1
				if endRow > maxRow then
					maxRow = endRow
				end
				for r = componentData.row, endRow do
					maxRowHeight[r] = math.max(maxRowHeight[r] or 0, componentData.rows)
				end
			end
...
local function createButton(name, text, callback, parentSection, gridPosition)
	...
	elseif parentSection and parentSection.Frame then
		...
		btn.Parent = parentSection or contentFrame
		return btn
	end
end
...
local enchantSection = createSection(childrenContainer, "Enchant", 5, false, false, "DO NOT hold anything in your hand.")
...
local function walkToTarget(target, config)
	...
	local function analyzeTerrainComplexity(startPos, targetPos)
		...
		for i = 1, samples do
			...
			if groundHit then
				...
				local obstacleHit = workspace:Raycast(samplePos + Vector3.new(0, 3, 0), (targetPos - samplePos).Unit * 5,
					rayParams)
				if obstacleHit then
		...
		local directHit = workspace:Raycast(startPos + Vector3.new(0, 3, 0), (targetPos - startPos), rayParams)
		if directHit then
	...
	local function updateTierPerformance(tier, success, timeSpent)
		...
		print(string.format("Tier %d Performance: %.1f%% success, %.1fs avg time, %d attempts", tier,
			perf.successRate * 100, perf.avgTime, perf.attempts))
	end
	...
	local function mainNavigationLoop()
			...
			if success then
				...
				if retries >= 2 then
					...
					if globalState.emergencyRetries <= 2 then
						...
						local escapePos = hrp.Position + escapeDir * 5 +
							Vector3.new((math.random() - 0.5) * 8, 0, (math.random() - 0.5) * 8)
		...
		for tier = 1, 3 do
			local perf = globalState.tierPerformance[tier]
			if perf.attempts > 0 then
				print(string.format("  Tier %d: %.1f%% success rate, %.1fs average time, %d attempts", tier,
					perf.successRate * 100, perf.avgTime, perf.attempts))
			end
		end
		...
		task.spawn(function()
			while AutoFarmState.active do
				...
				local function moveToLocation(targetCFrame, locationName)
					...
					elseif AutoFarmState.travelMode == "Teleport" then
...
local enchantRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Crafting"):WaitForChild("Enchant")
...
createButton("EnchantButton", "Enchant", function()
	local material = BackpackTwo:FindFirstChild(selectedMaterial)
	if material then
		...
		local ok, err = pcall(function()
			enchant = enchantRemote:InvokeServer(material)
		end)

		if ok then
			createNotification("Successfully enchanted with " .. enchant .. " using " .. selectedMaterial, 6)
		else
			createNotification("Error enchanting " .. selectedMaterial, 3)
			warn("Error enchanting " .. selectedMaterial .. ": " .. tostring(err))
		end
	else
...
createDropdown("EnchantSelector", "Target Enchant", enchantNames, "Prismatic", function(opt)
	targetEnchant = opt
	createNotification("Target enchant: " .. opt, 2)
end, enchantSection)

createButton("AutoEnchantButton", "Start Auto Enchant", function()
	if autoEnchanting then
		autoEnchanting = false
		createNotification("Stopped auto enchanting", 2)
		return
	end

	autoEnchanting = true
	createNotification("Auto enchanting until " .. targetEnchant, 3)
		...
		while autoEnchanting do
			...
			if material then
				...
				if ok then
					...
					createNotification("Error during auto enchant", 2)
					warn("Error enchanting: " .. tostring(err))
				end
			else
				autoEnchanting = false
				createNotification("Material not found: " .. selectedMaterial, 3)
...
createButton("EmergencyStopButton", "EMERGENCY STOP", function()
	autoEnchanting = false
	createNotification("EMERGENCY STOP - Auto enchanting terminated", 3)
end, enchantSection)
