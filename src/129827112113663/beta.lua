...
SimpleUI.Themes = {
    ...
    CozyWave = {
    PrimaryColor = Color3.fromRGB(30, 24, 44),
    SecondaryColor = Color3.fromRGB(48, 40, 66),
    SecondaryColorHover = Color3.fromRGB(60, 50, 84),
    SecondaryColorActive = Color3.fromRGB(72, 60, 104),
    SecondaryColorMouse1Down = Color3.fromRGB(38, 32, 56),

    TertiaryColor = Color3.fromRGB(54, 46, 76),
    TertiaryColorHover = Color3.fromRGB(66, 56, 94),
    TertiaryColorActive = Color3.fromRGB(80, 68, 120),
    TertiaryColorMouse1Down = Color3.fromRGB(44, 38, 62),

    TextInactive = Color3.fromRGB(190, 180, 210),
    TextActive = Color3.fromRGB(230, 220, 245),
    TextPrimary = Color3.fromRGB(240, 235, 255),
    TextSecondary = Color3.fromRGB(175, 165, 195),

    AccentColor = Color3.fromRGB(200, 150, 255),

    PrimaryFont = Enum.Font.GothamBlack,
    PrimaryFontSize = 18,
    SecondaryFont = Enum.Font.GothamMedium,
    SecondaryFontSize = 16,
...
SimpleUI.DefaultElements = {
    MainFrame = {
        ...
        Children = {{
            ...
            Properties = {
                MinSize = Vector2.new(0, 320),
                MaxSize = Vector2.new(math.huge, 550)
            }
        }}
    },
    TopBar = {
...
function SimpleUI:setTheme(window, themeName)
    if not window or not self.Themes[themeName] then
        return
    end
    local theme = self.Themes[themeName]
    window.Theme = theme

    local function applyThemeRecursive(element, config)
        if element:IsA("GuiObject") then
            if config and config.Theme then
                for prop, key in pairs(config.Theme) do
                    if theme[key] ~= nil and element[prop] ~= nil then
                        element[prop] = theme[key]
                    end
                end
            end
        end
        for _, child in ipairs(element:GetChildren()) do
            ...
            for _, cfg in pairs(self.DefaultElements) do
                if child.ClassName == cfg.Class then
                    childConfig = cfg
                    break
                end
            end
            applyThemeRecursive(child, childConfig)
        end
    end

    applyThemeRecursive(window.Elements.ScreenGui)

    for _, elem in ipairs(window.Elements.ScreenGui:GetDescendants()) do
        if elem.updateToggle then
            elem:updateToggle(false)
        end
        if elem.updateDisplay then
            elem:updateDisplay()
        end
    end
end
    ...
    if totalHeight > availableHeight then
        if not container:IsA("ScrollingFrame") then
            ...
            local props = {
                ...
                BorderSizePixel = container.BorderSizePixel,
                ZIndex = container.ZIndex,
                ScrollBarThickness = 0,
                CanvasSize = UDim2.new(0, 0, 0, totalHeight + paddingOffset),
                ScrollingDirection = Enum.ScrollingDirection.Y
            }
            local scroll = self:createElement("ScrollingFrame", props, parent)
            ...
            container.CanvasSize = UDim2.new(0, 0, 0, totalHeight + paddingOffset)
        end
    elseif container:IsA("ScrollingFrame") then
...
function SimpleUI:createWindow(options)
    options = options or {}
    local theme = options.Theme or self.Themes.CozyWave
    ...
    if titleAlignment == "Left" then
        titleContainerData.Properties.Position = UDim2.new(0, 10, 0.5, 0)
        titleContainerData.Properties.AnchorPoint = Vector2.new(0, 0.5)
        controlsContainerData.Properties.Position = UDim2.new(1, -10, 0.5, 0)
        controlsContainerData.Properties.AnchorPoint = Vector2.new(1, 0.5)
    elseif titleAlignment == "Right" then
        titleContainerData.Properties.Position = UDim2.new(1, -10, 0.5, 0)
        titleContainerData.Properties.AnchorPoint = Vector2.new(1, 0.5)
        controlsContainerData.Properties.Position = UDim2.new(0, 10, 0.5, 0)
        controlsContainerData.Properties.AnchorPoint = Vector2.new(0, 0.5)
    else
        titleContainerData.Properties.Position = UDim2.new(0.5, 0, 0.5, 0)
        titleContainerData.Properties.AnchorPoint = Vector2.new(0.5, 0.5)
        controlsContainerData.Properties.Position = UDim2.new(1, -10, 0.5, 0)
        controlsContainerData.Properties.AnchorPoint = Vector2.new(1, 0.5)
    end
    ...
    if hasIcon then
        ...
        if string.match(iconImage, "^https?://") then
            if getcustomasset and game.HttpGet then
    ...
    local textAlignment = titleAlignment == "Left" and Enum.TextXAlignment.Left or titleAlignment == "Right" and
                              Enum.TextXAlignment.Right or Enum.TextXAlignment.Center
    ...
    if tabsContainerData.ListLayout then
        self:createElement("UIListLayout", {
            ...
            Padding = UDim.new(0, tabsContainerData.Padding and tabsContainerData.Padding.Vertical or 0),
            HorizontalAlignment = tabsContainerData.ListLayout.HorizontalAlignment
        }, tabsContainer)
    end
    if tabsContainerData.Padding then
        local p = tabsContainerData.Padding.Horizontal
...
local function teleportToNextRune(label)
    local list = getRunes()
    if #list == 0 then
        label.SetSubText("No runes found in workspace")
        return
    end
    ...
    if not rune or not rune:IsA("Model") then
        label.SetSubText("Invalid rune")
        return
    end
    local target = rune:FindFirstChild("MainPart")
    if not target or not target:IsA("BasePart") then
        label.SetSubText("Rune has no MainPart")
        return
    end
    local char = Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then
        label.SetSubText("No HumanoidRootPart")
        return
    end
    root.CFrame = target.CFrame + Vector3.new(0, 3, 0)
    label.SetSubText(string.format("Teleported to rune %d/%d", currentRuneIndex, #list))
end
...
local WaypointsDropdown = SimpleUI:createDropdown(TeleportPage, "Select Waypoint", {}, nil, function(selection)
...
SimpleUI:createButton(TeleportPage, "Refresh waypoints", function()
    ...
    WaypointsDropdown.setOptions(waypointsTable)
end)
...
SimpleUI:createSlider(PlayerPage, "Super Jump Power", 50, 500, superJumpPower, function(val)
    pcall(function()
        superJumpPower = val
        if Humanoid then
...
SimpleUI:createButton(PlayerPage, "Super Jump", function()
    pcall(function()
        if Humanoid then
...
SimpleUI:createButton(PlayerPage, "Spoof Textures", function()
    pcall(function()
        if LocalCharacter then
            for _, part in pairs(LocalCharacter:GetDescendants()) do
                ...
                elseif part:IsA("Accessory") and part:FindFirstChild("Handle") then
                    for _, subPart in pairs(part.Handle:GetDescendants()) do
                        if subPart:IsA("Decal") then
                            subPart.Texture = ""
                        elseif subPart:IsA("Texture") then
                            subPart.Texture = ""
                        end
                    end
                end
...
local themeNames = {}
for name, _ in pairs(SimpleUI.Themes) do
