...
LoopFramework:registerTask("AutoBuySelectedSeedLoop", 0.5, function()
	if AutoBuySeedEnabled then
		for seed, price in pairs(SelectedSeeds) do
			buyItem(seed)
		end
	end
end)
...
LoopFramework:registerTask("AutoSellBrainrotLoop", 5, function()
	...
	print(string.format(
		"[AutoSell] Inventory: %d/%d | Filters -> Rarity[%s] Modifier[%s] Weight: %s %s",
		#inventory,
		maxSize,
		table.concat(rarityFilter, ", "),
		table.concat(modifierFilter, ", "),
		weightFilter.Direction or "Above",
		tostring(weightFilter.Weight or 0)
	))
...
createTextBox("AutoSellBrainrotWeightTextBox", "Enter Weight", nil, function(text)
	SelectedBrainrotWeightFilter = SelectedBrainrotWeightFilter or {}
